 ...
 *
 * Port of NATTEN's reference FNA kernels for Apple Silicon (MPS).
 * Implements forward and backward passes for 1D, 2D, and 3D neighborhood attention.
 * Pure ObjC++ with inline Metal shaders, no Swift bridge needed.
 */
...
// ---- Tiled FP32 kernel: Br=32 (D<=64) ----
// Grid:  (ceil(seqlen_q / 32), heads_q * batch_size)
// TGroup: 256 threads = 8 simdgroups of 32

kernel void na_forward_tiled_fp32_br32(
    device const float* Q        [[buffer(0)]],
    device const float* K        [[buffer(1)]],
    device const float* V        [[buffer(2)]],
    device float* O              [[buffer(3)]],
    ...
    // Br=32: each simdgroup owns 4 Q rows, 32 lanes = Bc=32
    constexpr int Br = 32;
    constexpr int Bc = 32;
    constexpr int ROWS_PER_SIMD = Br / NUM_SIMDGROUPS;  // 4
    ...
    // K: float[Bc][D], V: float[Bc][DV]
    threadgroup float smem_K[Bc * MAX_D];
    threadgroup float smem_V[Bc * MAX_D];

    // Alias per-simdgroup bounds onto smem_K (computed before K is loaded)
    // Layout: smem_sg_bounds[sg_id * 2 + 0] = min, smem_sg_bounds[sg_id * 2 + 1] = max
    threadgroup int* smem_sg_bounds = reinterpret_cast<threadgroup int*>(&smem_K[0]);

    // Each simdgroup owns ROWS_PER_SIMD Q rows
    ...
    for (int r = 0; r < ROWS_PER_SIMD; r++) {
        ...
        if (q_row < SQ) {
            ...
            for (int dd = 0; dd < D; dd++) {
                q_reg[r][dd] = Q[q_base + dd];
            }
        }
    }

    // ---- KV Bounding Box Computation ----
    int local_min_kv = SK;
    int local_max_kv = 0;
    ...
    // SIMD reduction within simdgroup, then each simdgroup writes to its slot
    int sg_min = simd_min(local_min_kv);
    int sg_max = simd_max(local_max_kv);

    if (simd_lane == 0) {
        smem_sg_bounds[simdgroup_id * 2 + 0] = sg_min;
        smem_sg_bounds[simdgroup_id * 2 + 1] = sg_max;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Serial reduction by thread 0
    int bbox_min_kv, bbox_max_kv;
    if (tid == 0) {
        bbox_min_kv = smem_sg_bounds[0];
        bbox_max_kv = smem_sg_bounds[1];
        for (int sg = 1; sg < NUM_SIMDGROUPS; sg++) {
            bbox_min_kv = min(bbox_min_kv, smem_sg_bounds[sg * 2 + 0]);
            bbox_max_kv = max(bbox_max_kv, smem_sg_bounds[sg * 2 + 1]);
        }
        smem_sg_bounds[0] = bbox_min_kv;
        smem_sg_bounds[1] = bbox_max_kv;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    int first_tile = bbox_min_kv / Bc;
    int last_tile = (bbox_max_kv + Bc - 1) / Bc;
    int num_tiles_total = (SK + Bc - 1) / Bc;
    last_tile = min(last_tile, num_tiles_total);

    // Also handle additional KV tokens: they're at indices [SQ, SQ + num_additional_kv)
    ...
    for (int tile_idx = first_tile; tile_idx < last_tile; tile_idx++) {
        ...
        // 256 threads load Bc * D values
        int total_k_elems = tile_len * D;
        for (int i = (int)tid; i < total_k_elems; i += NUM_THREADS) {
            int kk = i / D;  // which K position within tile
            int dd = i % D;  // which dim
            int global_k = kv_start + kk;
            int k_offset = batch_idx * SK * HK * D + global_k * HK * D + head_kv_idx * D + dd;
            smem_K[kk * D + dd] = K[k_offset];
        }

        // Cooperative load V tile into shared memory
        int total_v_elems = tile_len * DV;
        for (int i = (int)tid; i < total_v_elems; i += NUM_THREADS) {
            int kk = i / DV;
            int dd = i % DV;
            int global_k = kv_start + kk;
            int v_offset = batch_idx * SK * HK * DV + global_k * HK * DV + head_kv_idx * DV + dd;
            smem_V[kk * DV + dd] = V[v_offset];
        }

        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Each simdgroup processes its Q rows against the K tile
        for (int r = 0; r < ROWS_PER_SIMD; r++) {
            int q_row = q_indices[r];
            if (q_row >= SQ) continue;

            // Each lane computes score for one K position
            float score = -INFINITY;
            int k_pos_in_tile = (int)simd_lane;

            if (k_pos_in_tile < tile_len) {
                int global_k = kv_start + k_pos_in_tile;

                // Compute Q*K dot product from registers (Q) and shared memory (K)
                float acc = 0.0f;
                int d4 = D / 4;
                for (int dd = 0; dd < d4; dd++) {
                    float4 q4 = float4(q_reg[r][dd*4], q_reg[r][dd*4+1],
                                       q_reg[r][dd*4+2], q_reg[r][dd*4+3]);
                    float4 k4 = *reinterpret_cast<threadgroup const float4*>(&smem_K[k_pos_in_tile * D + dd * 4]);
                    acc += dot(q4, k4);
                }
                for (int dd = d4 * 4; dd < D; dd++) {
                    acc += q_reg[r][dd] * smem_K[k_pos_in_tile * D + dd];
                }
                acc *= params.attn_scale;
            ...
            // Online softmax: parallel reduction within simdgroup
            float tile_max = simd_max(score);

            if (tile_max == -INFINITY) continue;  // entire tile masked out
            ...
            if (tile_max > row_max[r]) {
                ...
                for (int dd = 0; dd < DV; dd++) {
                    o_acc[r][dd] *= correction;
                }
                row_max[r] = tile_max;
            }

            float exp_score = (score != -INFINITY) ? exp(score - row_max[r]) : 0.0f;
            float tile_sum = simd_sum(exp_score);
            row_sum[r] += tile_sum;

            // V accumulation: weighted sum reduced across lanes
            // Each lane holds exp_score for its K position, reads V from shared mem
            if (k_pos_in_tile < tile_len) {
                for (int dd = 0; dd < DV; dd += 4) {
                    int remaining = min(4, DV - dd);
                    if (remaining == 4) {
                        float4 v4 = *reinterpret_cast<threadgroup const float4*>(&smem_V[k_pos_in_tile * DV + dd]);
                        float4 weighted = exp_score * v4;
                        o_acc[r][dd]   += simd_sum(weighted.x);
                        o_acc[r][dd+1] += simd_sum(weighted.y);
                        o_acc[r][dd+2] += simd_sum(weighted.z);
                        o_acc[r][dd+3] += simd_sum(weighted.w);
                    } else {
                        for (int ddd = 0; ddd < remaining; ddd++) {
                            float v_val = smem_V[k_pos_in_tile * DV + dd + ddd];
                            o_acc[r][dd + ddd] += simd_sum(exp_score * v_val);
                        }
                    }
                }
            } else {
                // This lane doesn't have a valid K position, but still needs to
                // participate in simd_sum with zero contribution
                for (int dd = 0; dd < DV; dd += 4) {
                    int remaining = min(4, DV - dd);
                    if (remaining == 4) {
                        // exp_score is 0 for invalid lanes
                        simd_sum(0.0f);
                        simd_sum(0.0f);
                        simd_sum(0.0f);
                        simd_sum(0.0f);
    ...
    // Also process additional KV tiles if they exist and weren't covered by bbox
    if (has_additional) {
        for (int tile_idx = additional_first_tile; tile_idx < additional_last_tile; tile_idx++) {
            if (tile_idx >= first_tile && tile_idx < last_tile) continue;  // already processed
            ...
            for (int r = 0; r < ROWS_PER_SIMD; r++) {
                ...
                if (k_pos_in_tile < tile_len) {
                    ...
                    for (int dd = 0; dd < DV; dd += 4) {
                        int remaining = min(4, DV - dd);
                        if (remaining == 4) {
                            simd_sum(0.0f); simd_sum(0.0f); simd_sum(0.0f); simd_sum(0.0f);
                        } else {
                            for (int ddd = 0; ddd < remaining; ddd++) simd_sum(0.0f);
                        }
                    }
                }
            }
    ...
    // ---- Write output ----
    // Only lane 0 of each simdgroup writes (it has the reduced o_acc values)
    if (simd_lane == 0) {
        for (int r = 0; r < ROWS_PER_SIMD; r++) {
            int q_row = q_indices[r];
            if (q_row >= SQ) continue;
            ...
            float scale_val = (row_sum[r] > 0.0f) ? (1.0f / row_sum[r]) : 0.0f;
            for (int dd = 0; dd < DV; dd++) {
                O[o_base + dd] = o_acc[r][dd] * scale_val;
            }

            int lse_idx = batch_idx * SQ * H + q_row * H + head_q_idx;
            LSE[lse_idx] = log(row_sum[r]) + row_max[r];
        }
    }
}

// ---- Tiled FP32 kernel: Br=16 (D<=128) ----
// Grid:  (ceil(seqlen_q / 16), heads_q * batch_size)

kernel void na_forward_tiled_fp32_br16(
    device const float* Q        [[buffer(0)]],
    device const float* K        [[buffer(1)]],
    device const float* V        [[buffer(2)]],
    device float* O              [[buffer(3)]],
    ...
    constexpr int Br = 16;
    constexpr int Bc = 32;
    constexpr int ROWS_PER_SIMD = Br / NUM_SIMDGROUPS;  // 2
    ...
    int s_min = simd_min(local_min_kv);
    int s_max = simd_max(local_max_kv);

    if (simd_lane == 0) {
        smem_sg_bounds[simdgroup_id * 2 + 0] = s_min;
        smem_sg_bounds[simdgroup_id * 2 + 1] = s_max;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    bbox_max_kv = smem_sg_bounds[1];
    int first_tile = bbox_min_kv / Bc;
    int last_tile = min((bbox_max_kv + Bc - 1) / Bc, (SK + Bc - 1) / Bc);
...
// ---- Tiled FP16 kernel: Br=32 (D<=64) ----

kernel void na_forward_tiled_fp16_br32(
    device const half* Q         [[buffer(0)]],
    device const half* K         [[buffer(1)]],
    device const half* V         [[buffer(2)]],
    device half* O               [[buffer(3)]],
    ...
    // Shared memory stores float (promoted from half)
    threadgroup float smem_K[Bc * MAX_D];
    threadgroup float smem_V[Bc * MAX_D];
    threadgroup int* smem_sg_bounds = reinterpret_cast<threadgroup int*>(&smem_K[0]);
    ...
    int bbox_min_kv, bbox_max_kv;
    if (tid == 0) {
        bbox_min_kv = smem_sg_bounds[0]; bbox_max_kv = smem_sg_bounds[1];
        for (int sg = 1; sg < NUM_SIMDGROUPS; sg++) {
            bbox_min_kv = min(bbox_min_kv, smem_sg_bounds[sg * 2 + 0]);
            bbox_max_kv = max(bbox_max_kv, smem_sg_bounds[sg * 2 + 1]);
        }
        smem_sg_bounds[0] = bbox_min_kv; smem_sg_bounds[1] = bbox_max_kv;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    bbox_min_kv = smem_sg_bounds[0]; bbox_max_kv = smem_sg_bounds[1];
    int first_tile = bbox_min_kv / Bc;
    ...
    for (int tile_idx = first_tile; tile_idx < last_tile; tile_idx++) {
        ...
        // Load K (half -> float) into smem
        int total_k = tile_len * D;
        for (int i = (int)tid; i < total_k; i += NUM_THREADS) {
            int kk = i / D, dd = i % D;
            int global_k = kv_start + kk;
            smem_K[kk * D + dd] = float(K[batch_idx * SK * HK * D + global_k * HK * D + head_kv_idx * D + dd]);
        }
        int total_v = tile_len * DV;
        for (int i = (int)tid; i < total_v; i += NUM_THREADS) {
            int kk = i / DV, dd = i % DV;
            int global_k = kv_start + kk;
            smem_V[kk * DV + dd] = float(V[batch_idx * SK * HK * DV + global_k * HK * DV + head_kv_idx * DV + dd]);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        for (int r = 0; r < ROWS_PER_SIMD; r++) {
            ...
            if (k_pos < tile_len) {
                ...
                int d4 = D / 4;
                for (int dd = 0; dd < d4; dd++) {
                    float4 q4 = float4(q_reg[r][dd*4], q_reg[r][dd*4+1], q_reg[r][dd*4+2], q_reg[r][dd*4+3]);
                    float4 k4 = *reinterpret_cast<threadgroup const float4*>(&smem_K[k_pos * D + dd * 4]);
                    acc += dot(q4, k4);
                }
                for (int dd = d4 * 4; dd < D; dd++) acc += q_reg[r][dd] * smem_K[k_pos * D + dd];
                acc *= params.attn_scale;
                ...
                if (global_k >= SQ && global_k - SQ < params.num_additional_kv) {
                    ...
                    score = compute_na_mask(q_row, global_k, na_dim, params.qkv_shape, params.window_size,
                                            params.stride, params.dilation, params.is_causal) ? acc : -INFINITY;
                }
            }
            float tile_max = simd_max(score);
            if (tile_max == -INFINITY) continue;
            if (tile_max > row_max[r]) {
                float c = exp(row_max[r] - tile_max);
                row_sum[r] *= c;
                for (int dd = 0; dd < DV; dd++) o_acc[r][dd] *= c;
                row_max[r] = tile_max;
            }
            float es = (score != -INFINITY) ? exp(score - row_max[r]) : 0.0f;
            row_sum[r] += simd_sum(es);
            if (k_pos < tile_len) {
                for (int dd = 0; dd < DV; dd += 4) {
                    int rem = min(4, DV - dd);
                    if (rem == 4) {
                        float4 v4 = *reinterpret_cast<threadgroup const float4*>(&smem_V[k_pos * DV + dd]);
                        o_acc[r][dd]   += simd_sum(es * v4.x);
                        o_acc[r][dd+1] += simd_sum(es * v4.y);
                        o_acc[r][dd+2] += simd_sum(es * v4.z);
                        o_acc[r][dd+3] += simd_sum(es * v4.w);
                    } else { for (int ddd = 0; ddd < rem; ddd++) o_acc[r][dd+ddd] += simd_sum(es * smem_V[k_pos * DV + dd + ddd]); }
                }
            } else {
    ...
    if (simd_lane == 0) {
        for (int r = 0; r < ROWS_PER_SIMD; r++) {
            ...
            for (int dd = 0; dd < DV; dd++) O[o_base + dd] = half(o_acc[r][dd] * s);
            LSE[batch_idx * SQ * H + q_row * H + head_q_idx] = log(row_sum[r]) + row_max[r];
        }
    }
}

// ---- Tiled FP16 kernel: Br=16 (D<=128) ----

kernel void na_forward_tiled_fp16_br16(
    device const half* Q         [[buffer(0)]],
    device const half* K         [[buffer(1)]],
    device const half* V         [[buffer(2)]],
    device half* O               [[buffer(3)]],
    ...
    for (int tile_idx = first_tile; tile_idx < last_tile; tile_idx++) {
        ...
        for (int r = 0; r < ROWS_PER_SIMD; r++) {
            ...
            if (k_pos < tile_len) {
                ...
                for (int dd = d4 * 4; dd < D; dd++) acc += q_reg[r][dd] * smem_K[k_pos * D + dd];
                acc *= params.attn_scale;
                if (global_k >= SQ && global_k - SQ < params.num_additional_kv) { score = acc; }
                else if (global_k >= SQ) { score = -INFINITY; }
                else { score = compute_na_mask(q_row, global_k, na_dim, params.qkv_shape, params.window_size, params.stride, params.dilation, params.is_causal) ? acc : -INFINITY; }
            }
            float tile_max = simd_max(score);
            if (tile_max == -INFINITY) continue;
            if (tile_max > row_max[r]) { float c = exp(row_max[r] - tile_max); row_sum[r] *= c; for (int dd = 0; dd < DV; dd++) o_acc[r][dd] *= c; row_max[r] = tile_max; }
            float es = (score != -INFINITY) ? exp(score - row_max[r]) : 0.0f;
            row_sum[r] += simd_sum(es);
            if (k_pos < tile_len) {
                for (int dd = 0; dd < DV; dd += 4) { int rem = min(4, DV - dd); if (rem == 4) { float4 v4 = *reinterpret_cast<threadgroup const float4*>(&smem_V[k_pos * DV + dd]); o_acc[r][dd] += simd_sum(es*v4.x); o_acc[r][dd+1] += simd_sum(es*v4.y); o_acc[r][dd+2] += simd_sum(es*v4.z); o_acc[r][dd+3] += simd_sum(es*v4.w); } else { for (int ddd = 0; ddd < rem; ddd++) o_acc[r][dd+ddd] += simd_sum(es * smem_V[k_pos * DV + dd + ddd]); } }
            } else {
                for (int dd = 0; dd < DV; dd += 4) { int rem = min(4, DV - dd); if (rem == 4) { simd_sum(0.0f); simd_sum(0.0f); simd_sum(0.0f); simd_sum(0.0f); } else { for (int ddd = 0; ddd < rem; ddd++) simd_sum(0.0f); } }
            }
        }
...
// ---- Tiled BF16 kernel: Br=32 (D<=64) ----

kernel void na_forward_tiled_bf16_br32(
    device const bfloat* Q       [[buffer(0)]],
    device const bfloat* K       [[buffer(1)]],
    device const bfloat* V       [[buffer(2)]],
    device bfloat* O             [[buffer(3)]],
    ...
    if (simd_lane == 0) {
        for (int r = 0; r < ROWS_PER_SIMD; r++) {
            ...
            int o_base = batch_idx * SQ * H * DV + q_row * H * DV + head_q_idx * DV;
            float s = (row_sum[r] > 0.0f) ? (1.0f / row_sum[r]) : 0.0f;
            for (int dd = 0; dd < DV; dd++) O[o_base + dd] = bfloat(o_acc[r][dd] * s);
            LSE[batch_idx * SQ * H + q_row * H + head_q_idx] = log(row_sum[r]) + row_max[r];
        }
    }
}

// ---- Tiled BF16 kernel: Br=16 (D<=128) ----

kernel void na_forward_tiled_bf16_br16(
    device const bfloat* Q       [[buffer(0)]],
    device const bfloat* K       [[buffer(1)]],
    device const bfloat* V       [[buffer(2)]],
    device bfloat* O             [[buffer(3)]],
...
kernel void na_backward_dQ_fp32(
    ...
    for (int tile = 0; tile < num_kv_tiles; tile++) {
        ...
        for (int idx_K = tile_offset + (int)tid; idx_K < tile_end; idx_K += (int)NUM_THREADS) {
            ...
            // score = exp(qk - lse) * (dov - doo)
            scores[idx_K - tile_offset] = exp(acc_qk - lse_val) * (acc_dov - acc_doo);
        }
    ...
    for (int gqa = 0; gqa < gqa_ratio; gqa++) {
        ...
        for (int tile = 0; tile < num_q_tiles; tile++) {
            ...
            for (int idx_Q = tile_offset + (int)tid; idx_Q < tile_end; idx_Q += (int)NUM_THREADS) {
                ...
                float lse_val = LSE[batch_idx * SQ * H + idx_Q * H + head_q_idx];
                scores[idx_Q - tile_offset] = exp(acc_qk - lse_val) * (acc_dov - acc_doo);
            }
    ...
    for (int gqa = 0; gqa < gqa_ratio; gqa++) {
        ...
        for (int tile = 0; tile < num_q_tiles; tile++) {
            ...
            for (int idx_Q = tile_offset + (int)tid; idx_Q < tile_end; idx_Q += (int)NUM_THREADS) {
                ...
                scores[idx_Q - tile_offset] = exp(acc_qk - lse_val);
            }
...
static inline void compute_q_bounds_for_k(
    ...
    for (int d = 0; d < na_dim; d++) {
        ...
        if (is_causal[d]) {
            ...
            // Non-causal: Q range is [k_c - wl, k_c + wl] clamped (conservative)
            int eff = qkv_fix_dilation(qkv_shape[d], dilation_arr[d], k_di);
            int q_min_c = max(0, k_c - wl);
            int q_max_c = min(eff - 1, k_c + wl);
            q_sc[d] = q_min_c * dilation_arr[d] + k_di;
            q_ec[d] = min(q_max_c * dilation_arr[d] + k_di + 1, qkv_shape[d]);
        }
    }
...
kernel void FNAME( \
    ...
    if (simd_lane == 0) { smem_sg_bounds[simdgroup_id*2] = sg_mn; smem_sg_bounds[simdgroup_id*2+1] = sg_mx; } \
    threadgroup_barrier(mem_flags::mem_threadgroup); \
    int bbox_min, bbox_max; \
    if (tid == 0) { \
        bbox_min = smem_sg_bounds[0]; bbox_max = smem_sg_bounds[1]; \
        for (int sg = 1; sg < NUM_SIMDGROUPS; sg++) { bbox_min = min(bbox_min, smem_sg_bounds[sg*2]); bbox_max = max(bbox_max, smem_sg_bounds[sg*2+1]); } \
        smem_sg_bounds[0] = bbox_min; smem_sg_bounds[1] = bbox_max; \
    } \
    threadgroup_barrier(mem_flags::mem_threadgroup); \
    bbox_min = smem_sg_bounds[0]; bbox_max = smem_sg_bounds[1]; \
    int first_tile = bbox_min / Bc; \
    int last_tile = min((bbox_max + Bc - 1) / Bc, (SK + Bc - 1) / Bc); \
    \
    /* Main KV Tile Loop */ \
    for (int tile_idx = first_tile; tile_idx < last_tile; tile_idx++) { \
        int kv_start = tile_idx * Bc; \
        ...
        for (int r = 0; r < ROWS_PER_SIMD; r++) { \
            ...
            float P = (score != -INFINITY) ? exp(score - row_lse[r]) : 0.0f; \
...
kernel void FNAME( \
    ...
    threadgroup int* smem_sg_bounds = reinterpret_cast<threadgroup int*>(&smem_Q[0]); \
    \
    ...
    if (tid == 0) { \
        bbox_min_q = smem_sg_bounds[0]; bbox_max_q = smem_sg_bounds[1]; \
        for (int sg = 1; sg < NUM_SIMDGROUPS; sg++) { bbox_min_q = min(bbox_min_q, smem_sg_bounds[sg*2]); bbox_max_q = max(bbox_max_q, smem_sg_bounds[sg*2+1]); } \
        smem_sg_bounds[0] = bbox_min_q; smem_sg_bounds[1] = bbox_max_q; \
    } \
    threadgroup_barrier(mem_flags::mem_threadgroup); \
    bbox_min_q = smem_sg_bounds[0]; bbox_max_q = smem_sg_bounds[1]; \
    int first_tile = bbox_min_q / Bc; \
    int last_tile = min((bbox_max_q + Bc - 1) / Bc, (SQ + Bc - 1) / Bc); \
    \
    ...
    for (int gqa = 0; gqa < gqa_ratio; gqa++) { \
        ...
        for (int tile_idx = first_tile; tile_idx < last_tile; tile_idx++) { \
            ...
            for (int r = 0; r < ROWS_PER_SIMD; r++) { \
                ...
                \
                float P = (score != -INFINITY) ? exp(score - q_lse) : 0.0f; \
...
static void metal_na_backward_impl(
    ...
    @autoreleasepool {
        ...
        if (use_tiled_bwd) {
            // Select tile preset based on head dimension
            int Br;
            ...
            // Fallback if pipelines unavailable (e.g. BF16)
            if (!tiled_dq_pipeline || !tiled_dkdv_pipeline) {
                use_tiled_bwd = false;
            } else {
                // Di is computed inline in dQ/dKdV kernels (no separate kernel/barrier needed)
